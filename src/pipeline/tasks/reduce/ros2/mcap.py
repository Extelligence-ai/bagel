"""Reduce a ROS2 MCAP bag to only the data around detected events.

Writes MCAP by copying raw message records within the kept windows -- it never decodes
and re-encodes messages, so it does not need rosidl typesupport (the reason writing MCAP
via ``serialize_message`` fails). Event detection still uses the decoded path
(``to_duckdb``); only the write is a raw byte passthrough.
"""
