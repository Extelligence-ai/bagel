...
from bagel.di.types.data_source import resolve
from bagel.di.types.topic_sink import TopicSink, guess_host, guess_port
from bagel.pipeline import (
    base,
    batch,
    capabilities,
    lerobot,
...
from bagel.sink import startup
...
# Built-in POML capabilities ship inside the package; resolve them relative to
# this file so they load from an installed wheel, not just the repo checkout.
...
def describe_data_source(path: str, args: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    return poml(
        str(_AGENT_DIR / "describe" / "data_source.poml"),
        context={
...
def describe_topic(
    ...
    return poml(
        str(_AGENT_DIR / "describe" / "topic.poml"),
        context={
            "topic_name": topic,
...
def list_live_topics(
    ...
    Args:
        type_ (str): The type of `TopicSink` to use (e.g., ROS1, ROS2, PX4). For the full
            list of supported types, see `TopicSink` in `bagel/di/types/topic_sink.py`.
...
def run_poml_capability(
    ...
    Examples:
        As an LLM prompt:
            Run the capability './bagel/agent/examples/woof.poml' on the ROS2 bag
            './data/sample/ros2/mcap'.

        As a Python call:
            >>> run_poml_capability("./bagel/agent/examples/woof.poml", {"foo": "bar"})
...
def main(
    ...
    """Boot the MCP server, re-establishing any standing pipelines first.

    Each override falls back to the corresponding setting when omitted, so
    `bagel serve` with no flags behaves exactly like the previous entry point.
    """
